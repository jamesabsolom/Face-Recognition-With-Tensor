...
ds_validate = None
batch_size = 32
class_names = ["Ben Green", "Jae Knight"]
...
model.add(tf.keras.layers.Dense(128, input_shape = (8,), activation=tf.nn.leaky_relu))  # Creates the first neuron layer with 128 neurons and the activation 
model.add(tf.keras.layers.Dense(128, activation=tf.nn.leaky_relu))  # function of leaky relu which is the standard 
model.add(tf.keras.layers.Dense(2, activation="sigmoid"))                                 # Output with 10 possible values 0-9 with a softmax activation as its a 
